...
Usage:
    python bin/archive_images.py [--dry-run] [--limit N] [--issue N] [--include-index]
...
Options:
    ...
    --include-index Also process index.md
"""
...
def extract_image_uuid(url):
    ...
    # or: https://mcusercontent.com/.../_compresseds/UUID.ext
    match = re.search(r'/(?:images|_compresseds)/([a-f0-9-]+)\.([a-z]+)$', url, re.IGNORECASE)
    if match:
...
def process_file(file_path, results, dry_run=False, download_images=True, folder_name=None):
    """Process a single file, extracting and downloading images."""
    # Determine folder name for images
    if folder_name is None:
        issue_num = get_issue_number(file_path.name)
        if issue_num is None:
            print(f"Warning: Could not extract issue number from {file_path.name}")
            return None
        folder_name = f"issue-{issue_num}"
        display_name = f"Issue {issue_num}: {file_path.name}"
    else:
    ...
    images_dir = IMAGES_DIR / folder_name
    ...
    for url in image_urls:
        ...
        if not uuid:
            ...
            results.images_failed.append((folder_name, url, "Could not extract UUID"))
            continue

        local_filename = f"{uuid}.{ext}"
        local_path = images_dir / local_filename
        relative_path = f"/assets/archive-images/{folder_name}/{local_filename}"
        ...
        if download_images:
            ...
            if success:
                ...
                results.images_failed.append((folder_name, url, error))
        else:
    ...
    for url in redirect_urls:
        ...
        if final_url:
            ...
            results.redirects_failed.append((folder_name, url, error))
...
def main():
    ...
    parser.add_argument('--include-index', action='store_true', help="Also process index.md")
    ...
    files_to_process = [(f, None) for f in issue_files]  # (file, folder_name)
    
    # Add index.md if requested
    index_file = ROOT_DIR / "index.md"
    if args.include_index and index_file.exists():
        files_to_process.append((index_file, "index"))
        print("Including index.md")
    ...
    # Process each file
    images_processed = 0
    for file_path, folder_name in files_to_process:
    ...
    if results.images_failed or results.redirects_failed:
        ...
        with open(report_path, 'w') as f:
            ...
            if results.images_failed:
                ...
                for folder, url, error in results.images_failed:
                    f.write(f"{folder}: {url}\n  Error: {error}\n\n")

            if results.redirects_failed:
                f.write("## Failed Redirects\n")
                for folder, url, error in results.redirects_failed:
                    f.write(f"{folder}: {url}\n  Error: {error}\n\n")
